...
function faqSections(sections){
  return sections.map(section => `
    <div class="faq-card">
      <div class="faq-title"><span class="faq-title-icon">${section.icon || "?"}</span>${section.title}</div>
      <div class="faq-list">
...
faqProvider: ()=> faqSections([
  {
    title:"Aggregate Model Management",
    items:[
  ...
  },
  {
    title:"Tenant Management",
    items:[
      {status:"Answered", kind:"partial", q:"How does AGIOne enforce TPS/token limits or rate-limiting for specific users or API keys?", a:"AGIOne currently supports rate limiting at the tenant and model level in the new version. During model publishing, the Provider configures the published model's billing and limit settings for the target tenant or model service. Operator-side governance defines the available tenant, model and quota boundaries. This controls TPS or token consumption at the tenant-model level. AGIOne does not currently support rate limiting based on an individual API key.", demo:"Publish Model / Rate Limit", image:"screenshots/d3-pr3-billing.png", imageAlt:"Model billing and limit configuration"},
